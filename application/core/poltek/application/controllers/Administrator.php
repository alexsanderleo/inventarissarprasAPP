...
class Administrator extends CI_Controller
{
    ...
    function tambah_listberita()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|jpeg|PNG|jpeg|PNG';
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'id_kategori' => $this->db->escape_str($this->input->post('a')),
                    'username' => $this->session->username,
    ...
    function edit_komentarberita()
    {
        ...
        if (isset($_POST['submit'])) {
            $data = array(
                'nama_komentar' => $this->input->post('a'),
    ...
    function tambah_testimoni()
    {
        cek_session_akses('testimoni', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_testimoni/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|jpeg|PNG|swf';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            redirect($this->uri->segment(1) . '/testimoni');
        } else {
    ...
    // Controller Modul Mitra

    function mitra()
    {
        cek_session_akses('mitra', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('mitra', 'id_mitra', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('mitra', array('username' => $this->session->username), 'id_mitra', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_mitra/view_mitra', $data);
    }
    ...
    function tambah_mitra()
    {
        cek_session_akses('mitra', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_mitra/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|jpeg|PNG|swf';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            $this->model_app->insert('mitra', $data);
            redirect($this->uri->segment(1) . '/mitra');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_mitra/view_mitra_tambah');
        }
    }
    ...
    function edit_mitra()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_mitra' => $this->input->post('id'));
            $this->model_app->update('mitra', $data, $where);
            redirect($this->uri->segment(1) . '/mitra');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('mitra', array('id_mitra' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('mitra', array('id_mitra' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_mitra/view_mitra_edit', $data);
        }
    }
    ...
    function delete_mitra()
    {
        cek_session_akses('mitra', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_mitra' => $this->uri->segment(3));
        } else {
            $id = array('id_mitra' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('mitra', $id);
        redirect($this->uri->segment(1) . '/mitra');
    }
    ...
    function tambah_sekilasinfo()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'info' => $this->input->post('a'),
                    'judul' => $this->db->escape_str($this->input->post('c')),
    ...
    function detail_pesanmasuk()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $this->email->from('admin@poltekkaltara.ac.id', 'Poltekkaltara.ac.id');
            $this->email->to($email);
            $this->email->cc('');
            $this->email->bcc('');
